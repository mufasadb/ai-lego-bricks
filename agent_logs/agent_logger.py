...
class AgentLogger:
    ...
    def log_stage(
        self, stage: str, message: str, data: Optional[Dict[str, Any]] = None
    ):
    ...
    def log_model_switch(self, from_model: str, to_model: str, purpose: str):
        ...
        self.log_stage(
            "MODEL_SWITCH",
            f"Switching from {from_model} to {to_model}",
            {"from_model": from_model, "to_model": to_model, "purpose": purpose},
        )
    ...
    def log_error(
        self, stage: str, error: Exception, context: Optional[Dict[str, Any]] = None
    ):
        """Log an error event"""
        self.log_stage(
            f"{stage}_ERROR",
