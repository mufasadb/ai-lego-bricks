...
def view_log_file(log_file, filter_stage=None, show_responses=True):
    ...
    try:
        ...
        if filter_stage:
            filtered_entries = [e for e in entries if filter_stage.lower() in e.get('stage', '').lower()]
        
        if not show_responses:
            # Remove model responses for cleaner view
            for entry in filtered_entries:
                if 'data' in entry and 'response' in entry['data']:
                    entry['data'] = {k: v for k, v in entry['data'].items() if k != 'response'}
...
def main():
    ...
    parser.add_argument("--no-responses", "-n", action="store_true", help="Hide model responses")
