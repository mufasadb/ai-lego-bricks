...
class StreamingDetector:
    ...
    def _is_streaming_step(step: StepConfig) -> bool:
        ...
        return step.type in [StepType.LLM_CHAT, StepType.TTS] and step.config.get(
            "stream", False
        )
        ...
        for step in workflow.steps:
            for input_config in step.inputs.values():
                if (
                    ...
                    and input_config.get("from_step") == step_id
                ):
        ...
        for step in self.workflow.steps:
            ...
            for other_step in self.workflow.steps:
                if (
                    ...
                    and other_step.id not in processed_steps
                    and self.dependency_graph[other_step.id] == step_deps
                ):
...
class AsyncWorkflowExecutor:
    ...
    async def execute(
        self, workflow: WorkflowConfig, initial_inputs: Optional[Dict[str, Any]] = None
    ) -> WorkflowResult:
        ...
        try:
            ...
            if (
                self.parallelization_mode == ParallelizationMode.DISABLED
                or streaming_analysis.streaming_mode == StreamingMode.ACTIVE
            ):
                ...
                return await self._execute_parallel(
                    workflow, initial_inputs or {}, streaming_analysis
                )
    ...
    async def _execute_parallel_group(
        self, parallel_group: ParallelGroup, workflow: WorkflowConfig
    ):
    ...
    async def _execute_step_async(self, step: StepConfig) -> Any:
        ...
        result = await loop.run_in_executor(
            None, handler, step, resolved_inputs, self.context
        )
