...
class ConversationThread(BaseModel):
    ...
    def get_last_messages(
        self, count: int, include_system: bool = True
...
class StreamBufferConfig(BaseModel):
    ...
    forward_on: str = Field(
        default="sentence",
        description="When to forward buffered content: 'sentence', 'time', 'chunk_size', 'word_count', 'immediate'",
    )
    max_buffer_time: float = Field(
        default=2.0,
        description="Maximum time to buffer before forcing forward (seconds)",
    )
    chunk_size: int = Field(
        default=50, description="Forward after N characters (for chunk_size strategy)"
    )
    word_count: int = Field(
        default=10, description="Forward after N words (for word_count strategy)"
    )
    sentence_count: int = Field(
        default=1,
        description="Forward after N complete sentences (for sentence strategy)",
    )
    min_chunk_length: int = Field(
        default=10, description="Minimum characters before considering forwarding"
    )
    pass_through_streams: bool = Field(
        default=False, description="For multi-agent: forward underlying agent streams"
    )
    buffer_overflow_strategy: str = Field(
        default="force_forward",
        description="What to do when buffer is full: 'force_forward', 'drop_oldest', 'block'",
    )
...
class ToolCallConfig(BaseModel):
    ...
    tools: Optional[List[str]] = Field(
        default=None, description="Specific tool names to use"
    )
    tool_category: Optional[str] = Field(
        default=None, description="Tool category to use"
    )
    ...
    auto_execute: bool = Field(
        default=True, description="Automatically execute tool calls"
    )
    prompt: Optional[str] = Field(
        default=None, description="System prompt for tool calling"
    )
    temperature: float = Field(default=0.1, description="Temperature for tool calling")
    thinking_tokens_mode: ThinkingTokensMode = Field(
        default=ThinkingTokensMode.HIDE, description="How to handle thinking tokens"
    )
...
class AgentReference(BaseModel):
    ...
    inherit_context: bool = Field(
        default=True, description="Whether to share execution context"
    )
    return_outputs: List[str] = Field(
        default_factory=list, description="Specific outputs to return from child agent"
    )
...
class StepConfig(BaseModel):
    ...
    routes: Optional[Dict[str, str]] = Field(
        default_factory=dict
    )  # Maps condition results to next step IDs
    max_iterations: Optional[int] = (
        None  # Maximum number of times this step can be executed
    )
    preserve_previous_results: bool = (
        False  # Whether to preserve results from previous iterations
    )
...
class WorkflowGlobalConfig(BaseModel):
    ...
    max_iterations: int = (
        10  # Default maximum iterations per step for loop-back protection
    )
    # Thinking tokens configuration
    thinking_tokens_mode: ThinkingTokensMode = ThinkingTokensMode.AUTO
    thinking_tokens_delimiters: List[str] = Field(
...
class ExecutionContext(BaseModel):
    ...
    step_iteration_counts: Dict[str, int] = Field(
        default_factory=dict
    )  # step_id -> iteration count
    step_iteration_history: Dict[str, List[Any]] = Field(
        default_factory=dict
    )  # step_id -> list of previous results
    ...
    agent_call_stack: List[str] = Field(
        default_factory=list
    )  # Track nested agent calls for recursion prevention
    parent_context: Optional["ExecutionContext"] = (
        None  # Reference to parent context for nested agents
    )
    agent_outputs: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict
    )  # agent_id -> outputs mapping

    def get_active_conversation(self) -> Optional[ConversationThread]:
        """Get the currently active conversation thread"""
        if (
            self.active_conversation_id
            and self.active_conversation_id in self.conversations
        ):
            return self.conversations[self.active_conversation_id]
        return None

    def create_conversation(
        self, conversation_id: str, name: Optional[str] = None
    ) -> ConversationThread:
