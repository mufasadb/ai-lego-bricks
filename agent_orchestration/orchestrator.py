...
def _safe_import(relative_path, absolute_path, item_name):
    ...
    try:
        module = __import__(relative_path, fromlist=[item_name])
...
class AgentOrchestrator:
            ...
            try:
                ...
                if self.config and self.config.config:
                    ...
                    tts_provider = getattr(
                        self.config.config, "default_tts_provider", "auto"
                    )
        ...
        if create_stt_service:
            try:
                ...
                if self.config and self.config.config:
                    ...
                    stt_provider = getattr(
                        self.config.config, "default_stt_provider", "auto"
                    )
    ...
    def execute_workflow(
        ...
        if workflow_file_path:
            ...
            executor.context.global_variables["_workflow_file_path"] = (
                workflow_file_path
            )
...
class WorkflowExecutor:
    ...
    def execute(
        self, workflow: WorkflowConfig, initial_inputs: Dict[str, Any]
    ) -> WorkflowResult:
        ...
        try:
            ...
            while current_step_index < len(workflow.steps):
                ...
                if iteration_count > max_iterations:
                    raise RuntimeError(
                        f"Maximum iterations ({max_iterations}) exceeded for step: {step.id}"
                    )
                ...
                if step.outputs:
                    for output_name in step.outputs:
                        if output_name in result:
                            self.context.step_outputs[f"{step.id}.{output_name}"] = (
                                result[output_name]
                            )
                        else:
        ...
        for key, value in inputs.items():
            if isinstance(value, dict) and "from_step" in value:
                ...
                if full_key in self.context.step_outputs:
                    ...
                    raise ValueError(
                        f"Could not resolve input reference: {step_id}.{field}"
                    )
            else:
    ...
    def _resolve_variable_reference(self, value: Any) -> Any:
        ...
        if isinstance(value, str) and value.startswith("$"):
            ...
            if var_name.startswith("iteration_context."):
                return self._resolve_iteration_context(
                    var_name[18:]
                )  # Remove "iteration_context." prefix
    ...
    def _resolve_iteration_context(self, context_key: str) -> Any:
        ...
        if context_key == "iteration_count":
            return (
                self.context.get_step_iteration_count(current_step_id)
                if current_step_id
                else 0
            )
        elif context_key == "previous_result":
            return (
                self.context.get_previous_step_result(current_step_id)
                if current_step_id
                else None
            )
        elif context_key == "iteration_history":
            return (
                self.context.get_step_iteration_history(current_step_id)
                if current_step_id
                else []
            )
    ...
    def _get_next_step(
        self, current_step: StepConfig, step_result: Any, workflow: WorkflowConfig
    ) -> Optional[str]:
