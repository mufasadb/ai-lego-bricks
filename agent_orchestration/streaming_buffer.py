...
class StreamBuffer:
    ...
    def _forward_on_sentences(self) -> List[str]:
        ...
        if len(sentences) >= self.config.sentence_count:
            ...
            for sentence in to_forward:
                ...
                self.buffer = re.sub(
                    sentence_pattern + r"\s*", "", self.buffer, count=1
                )
    ...
    def _forward_on_time(self) -> List[str]:
        ...
        if (
            time.time() - self.last_forward_time > self.config.max_buffer_time
            and len(self.buffer) >= self.config.min_chunk_length
        ):
            return self._force_forward()
        return []
...
def create_streaming_generator(
    source_generator: Generator[str, None, str], buffer_config: StreamBufferConfig
