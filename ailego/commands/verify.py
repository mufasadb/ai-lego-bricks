...
def _check_environment() -> Tuple[str, str]:
    ...
    llm_providers = ["GOOGLE_API_KEY", "ANTHROPIC_API_KEY", "OPENAI_API_KEY"]
...
def _check_project_structure() -> Tuple[str, str]:
    ...
    has_config = (current_dir / ".env").exists() or (
    ...
    if has_agents and has_config:
        ...
        return (
            "⚠ INFO",
            "Not in AI Lego Bricks project (run 'ailego init' to create one)",
        )
    ...
    for component, status, details in results:
        if status.startswith("✗") or status.startswith("⚠"):
            if "Python" in component:
                ...
                recommendations.append(
                    "• Run: ailego init <project-name> to create a project"
                )
