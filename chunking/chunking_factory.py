...
class ChunkingServiceFactory:
    ...
    def create_service(self, config: Optional[ChunkingConfig] = None) -> ChunkingService:
        """
    ...
    def _validate_config_parameter(self, key: str, value: Any) -> Any:
        ...
        if key == 'target_size':
            if not isinstance(value, int) or value <= 0:
                raise ValueError(f"target_size must be a positive integer, got: {value}")
        elif key == 'tolerance':
            if not isinstance(value, int) or value < 0:
                raise ValueError(f"tolerance must be a non-negative integer, got: {value}")
