...
def main():
    ...
    parser.add_argument(
        "--save-debug", action="store_true", help="Save debug log to file"
    )
    parser.add_argument(
        "--step-pause", action="store_true", help="Pause after each step"
    )
    ...
    try:
        ...
        with open(args.agent_file, "r") as f:
            ...
            if args.debug_level >= 2:
                print(
                    f"   Global Config: {json.dumps(config.get('config', {}), indent=4)}"
                )
        ...
        if result.success:
            ...
            if args.save_debug:
                ...
                debug_data = {
                    ...
                    "step_outputs": (
                        result.step_outputs if hasattr(result, "step_outputs") else {}
                    ),
