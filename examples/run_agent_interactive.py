    ...
    if result.success:
        ...
        if isinstance(result.final_output, dict):
            for key, value in result.final_output.items():
                if 'audio' in key.lower() and isinstance(value, str) and value.endswith('.wav'):
            ...
            for step_id, output in result.step_outputs.items():
                if isinstance(output, dict):
                    for key, value in output.items():
                        if 'file_path' in key and isinstance(value, str) and os.path.exists(value):
                            print(f"💾 Generated file: {value}")
    
    return result

def main():
    parser = argparse.ArgumentParser(description="Interactive agent runner for special scenarios")
    ...
    parser.add_argument("--voice-mode", action="store_true", help="Enable voice processing mode")
    parser.add_argument("--file-mode", action="store_true", help="Enable file processing mode")
    ...
    try:
        ...
        if args.save_session and result.success:
            ...
            session_data = {
                "agent": agent_name,
                "timestamp": timestamp,
                "mode": "stream" if args.stream else "voice" if args.voice_mode else "file" if args.file_mode else "interactive",
                "inputs": inputs,
