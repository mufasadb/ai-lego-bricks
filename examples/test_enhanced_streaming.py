...
try:
    ...
    buffer_path = os.path.join(
        project_root, "agent_orchestration", "streaming_buffer.py"
    )
...
def test_sentence_buffering():
    ...
    config = StreamBufferConfig(
        forward_on="sentence", sentence_count=1, max_buffer_time=2.0, min_chunk_length=5
    )
...
def test_word_count_buffering():
    ...
    config = StreamBufferConfig(
        forward_on="word_count", word_count=5, max_buffer_time=2.0, min_chunk_length=5
    )
...
def test_immediate_buffering():
    ...
    config = StreamBufferConfig(forward_on="immediate")
