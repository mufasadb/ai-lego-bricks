...
class OpenAIImageGenerationClient(ImageGenerationClient):
    def __init__(self, config: ImageGenerationConfig, credential_manager: Optional[CredentialManager] = None):
        ...
        self.api_key = self.credential_manager.require_credential("OPENAI_API_KEY", "OpenAI Image Generation")
...
class GoogleImageGenerationClient(ImageGenerationClient):
    def __init__(self, config: ImageGenerationConfig, credential_manager: Optional[CredentialManager] = None):
        super().__init__(config)
        self.credential_manager = credential_manager or default_credential_manager
        self.api_key = self.credential_manager.require_credential("GOOGLE_AI_STUDIO_KEY", "Google Image Generation")
...
class StabilityAIImageGenerationClient(ImageGenerationClient):
    def __init__(self, config: ImageGenerationConfig, credential_manager: Optional[CredentialManager] = None):
        super().__init__(config)
        self.credential_manager = credential_manager or default_credential_manager
        self.api_key = self.credential_manager.require_credential("STABILITY_API_KEY", "Stability AI Image Generation")
        self.base_url = "https://api.stability.ai/v1/generation"
        self.model = config.model or os.getenv("STABILITY_AI_MODEL", "stable-diffusion-xl-1024-v1-0")
        ...
        try:
            ...
            for i, artifact in enumerate(data.get("artifacts", [])):
                if artifact.get("finishReason") == "SUCCESS":
                    ...
                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                    filename = f"stability_image_{timestamp}_{uuid.uuid4().hex[:8]}_{i}.png"
                    filepath = os.path.join(self.config.output_dir, filename)
...
class LocalImageGenerationClient(ImageGenerationClient):
    def __init__(self, config: ImageGenerationConfig, credential_manager: Optional[CredentialManager] = None):
        ...
        self.model = config.model or os.getenv("LOCAL_IMAGE_GEN_DEFAULT_MODEL", "stable-diffusion")
