...
from .llm_types import (
    LLMProvider, VisionProvider, LLMConfig, VisionConfig,
...
class LLMClientFactory:
    ...
    @staticmethod
    def create_vision_client(provider: VisionProvider, model: Optional[str] = None,
                           temperature: float = 0.1, max_tokens: int = 2048,
                           **kwargs) -> VisionLLMClient:
        """
    ...
    @staticmethod
    def create_structured_client(provider: LLMProvider, schema: Type[T], 
                               model: Optional[str] = None, temperature: float = 0.7, 
                               max_tokens: int = 1000, 
                               config: Optional[StructuredResponseConfig] = None,
                               **kwargs) -> StructuredLLMWrapper[T]:
        """
    ...
    @staticmethod
    def create_structured_client_from_client(client: TextLLMClient, schema: Type[T],
                                           config: Optional[StructuredResponseConfig] = None) -> StructuredLLMWrapper[T]:
        """
    ...
    @staticmethod
    def switch_client_model(client: Union[TextLLMClient, VisionLLMClient], new_model: str) -> bool:
        """
        ...
        if hasattr(client, 'switch_model'):
            ...
            raise ValueError(f"Client {type(client).__name__} does not support model switching")
    ...
    @staticmethod
    def create_switchable_text_client(provider: LLMProvider, initial_model: Optional[str] = None,
                                     temperature: float = 0.7, max_tokens: int = 1000,
                                     **kwargs) -> TextLLMClient:
        """
        ...
        if provider not in [LLMProvider.OLLAMA, LLMProvider.ANTHROPIC]:
            raise ValueError(f"Model switching only supported for Ollama and Anthropic providers, not {provider}")
        
        return LLMClientFactory.create_text_client(provider, initial_model, temperature, max_tokens, **kwargs)
    
    @staticmethod
    def create_switchable_vision_client(provider: VisionProvider, initial_model: Optional[str] = None,
                                       temperature: float = 0.1, max_tokens: int = 2048,
                                       **kwargs) -> VisionLLMClient:
        """
        ...
        if provider != VisionProvider.LLAVA:
            raise ValueError(f"Model switching only supported for LLaVA provider, not {provider}")
        
        return LLMClientFactory.create_vision_client(provider, initial_model, temperature, max_tokens, **kwargs)
    
    @staticmethod
    def create_generation_service(provider: LLMProvider, model: Optional[str] = None,
                                temperature: float = 0.7, max_tokens: int = 1000,
    ...
    @staticmethod
    def create_conversation_service(provider: LLMProvider, model: Optional[str] = None,
                                  temperature: float = 0.7, max_tokens: int = 1000,
                                  conversation_id: Optional[str] = None,
                                  **kwargs) -> 'ConversationService':
        """
...
def create_switchable_text_client(provider: str, **kwargs) -> TextLLMClient:
    ...
    return LLMClientFactory.create_switchable_text_client(LLMProvider(provider), **kwargs)
...
def create_switchable_vision_client(provider: str, **kwargs) -> VisionLLMClient:
    ...
    return LLMClientFactory.create_switchable_vision_client(VisionProvider(provider), **kwargs)


# Structured response convenience functions
def create_structured_client(provider: str, schema: Type[T], **kwargs) -> StructuredLLMWrapper[T]:
    """Convenience function to create structured client from string"""
    return LLMClientFactory.create_structured_client(LLMProvider(provider), schema, **kwargs)

def create_structured_client_from_client(client: TextLLMClient, schema: Type[T], **kwargs) -> StructuredLLMWrapper[T]:
    """Convenience function to wrap existing client with structured responses"""
    return LLMClientFactory.create_structured_client_from_client(client, schema, **kwargs)


# New service convenience functions
def create_generation_service(provider: str, model: Optional[str] = None, **kwargs) -> 'GenerationService':
    """Convenience function to create generation service from string"""
    return LLMClientFactory.create_generation_service(LLMProvider(provider), model, **kwargs)


def create_conversation_service(provider: str, model: Optional[str] = None, **kwargs) -> 'ConversationService':
    """Convenience function to create conversation service from string"""
    return LLMClientFactory.create_conversation_service(LLMProvider(provider), model, **kwargs)


# Provider-specific convenience functions
def create_ollama_generation(model: Optional[str] = None, **kwargs) -> 'GenerationService':
    """Create Ollama generation service"""
    return create_generation_service('ollama', model, **kwargs)


def create_gemini_generation(model: Optional[str] = None, **kwargs) -> 'GenerationService':
    """Create Gemini generation service"""
    return create_generation_service('gemini', model, **kwargs)


def create_anthropic_generation(model: Optional[str] = None, **kwargs) -> 'GenerationService':
    """Create Anthropic generation service"""
    return create_generation_service('anthropic', model, **kwargs)


def create_ollama_conversation(model: Optional[str] = None, **kwargs) -> 'ConversationService':
    """Create Ollama conversation service"""
    return create_conversation_service('ollama', model, **kwargs)


def create_gemini_conversation(model: Optional[str] = None, **kwargs) -> 'ConversationService':
    """Create Gemini conversation service"""
    return create_conversation_service('gemini', model, **kwargs)


def create_anthropic_conversation(model: Optional[str] = None, **kwargs) -> 'ConversationService':
