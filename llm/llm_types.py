...
class StructuredLLMWrapper:
    ...
    def __init__(self, client: TextLLMClient, schema: Type[T], config: Optional[StructuredResponseConfig] = None):
        self.client = client
        self.schema = schema
    ...
    def _enforce_structured_response(self, message: str, chat_history: Optional[List[ChatMessage]] = None) -> T:
            ...
            try:
                ...
                if attempt == self.config.retry_attempts - 1:
                    if self.config.fallback_to_text:
                        ...
                        return self._create_fallback_response(response if 'response' in locals() else str(e))
                    else:
                        raise ValueError(f"Failed to get structured response after {self.config.retry_attempts} attempts: {e}")
                
                # Modify prompt for retry
                enhanced_prompt = self._build_retry_prompt(message, schema_json, attempt + 1)
...
"""
    ...
    def _build_retry_prompt(self, message: str, schema: Dict[str, Any], attempt: int) -> str:
...
"""
    ...
    def _create_fallback_response(self, failed_response: str) -> T:
        ...
        try:
                ...
                for field_name, field_info in schema['properties'].items():
                    if field_name in schema.get('required', []):
                        ...
                        if field_info.get('type') == 'string':
                            default_dict[field_name] = failed_response[:100] if failed_response else "Error: Could not parse response"
            ...
            raise ValueError(f"Could not create structured response from: {failed_response}")
