        ...
        for attempt in range(max_retries):
            try:
                ...
                if e.response.status_code == 503 and attempt < max_retries - 1:
                    ...
                    print("=" * 80)
                    print(
                        "🚨 CRITICAL: GEMINI VISION API QUOTA/BILLING LIMIT EXCEEDED! 🚨"
                    )
                    print("=" * 80)
                    ...
                    print(
                        "   • Agents can rapidly consume vision API quotas without user awareness"
                    )
                    print(
                        "   • Vision processing is particularly expensive - monitor usage closely"
                    )
                    print(
...
class LLaVAClient(VisionLLMClient):
    ...
    def __init__(self, config: VisionConfig):
        ...
        self.base_url = os.getenv(
            "OLLAMA_URL", os.getenv("LLAVA_URL", "http://localhost:11434")
        )
    ...
    def _verify_llava_available(self):
        ...
        try:
            with httpx.Client(timeout=10.0) as client:
                ...
                vision_models = [
                    m for m in available_models if self._is_vision_model(m)
                ]
                if not vision_models:
                    raise RuntimeError(
                        f"No vision models found. Available models: {available_models}"
                    )
