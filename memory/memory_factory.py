...
class MemoryServiceFactory:
    ...
    def create_memory_service(
        ...
        **kwargs,
        ...
        if service_type == "supabase":
            return MemoryServiceFactory._create_supabase_service(
                credential_manager, **kwargs
            )
        elif service_type == "neo4j":
            return MemoryServiceFactory._create_neo4j_service(
                credential_manager, **kwargs
            )
        else:
    ...
    def _detect_available_service(
        ...
        neo4j_uri = cred_manager.get_credential(
            "NEO4J_URI"
        ) or cred_manager.get_credential("NEO4J_URL")
        if neo4j_uri:
            return "neo4j"
    ...
    def _create_supabase_service(
        ...
        try:
            ...
            return SupabaseMemoryService(
                credential_manager=credential_manager, **kwargs
            )
        except ImportError as e:
            raise ImportError(f"Failed to import Supabase dependencies: {e}")

    @staticmethod
    def _create_neo4j_service(
        credential_manager: Optional[CredentialManager] = None, **kwargs
    ) -> MemoryService:
...
def create_memory_service(
    ...
    **kwargs,
    ...
    return MemoryServiceFactory.create_memory_service(
        service_type, credential_manager, **kwargs
    )
