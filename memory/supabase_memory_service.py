...
class SupabaseMemoryService(MemoryService):
    ...
    def __init__(self, supabase_url: str = None, supabase_key: str = None, table_name: str = "memories", credential_manager: Optional['CredentialManager'] = None):
        ...
        if EMBEDDING_ABSTRACTION_AVAILABLE:
            ...
            logger.info(f"Using direct embedding model: {embedding_model} with {self.embedding_dim} dimensions")
    ...
    def _verify_pgvector_setup(self):
        ...
        try:
            ...
            if result.data and result.data[0].get('has_function'):
                ...
                logger.warning("⚠ match_memories function not found - falling back to text search")
    ...
    def retrieve_memories(self, query: str, limit: int = 10) -> List[Memory]:
        ...
        try:
            ...
            result = self.supabase.table(self.table_name).select("*").ilike('content', f'%{query}%').order('created_at', desc=True).limit(limit).execute()
    ...
    def update_memory(self, memory_id: str, content: str, metadata: Optional[Dict[str, Any]] = None) -> bool:
        ...
        try:
            ...
            result = self.supabase.table(self.table_name).update(update_data).eq('id', memory_id).execute()
            return len(result.data) > 0
