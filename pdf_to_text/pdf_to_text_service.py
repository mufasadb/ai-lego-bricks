...
class PDFExtractOptions(BaseModel):
    ...
    page_range: Optional[tuple] = Field(
        default=None, description="Page range as (start, end) tuple"
    )

    # Vision-based Processing Options (for image-heavy PDFs)
    use_vision_fallback: bool = Field(
        default=False, description="Use vision models when text extraction fails"
    )
    force_vision_processing: bool = Field(
        default=False, description="Force use of vision models even for text-based PDFs"
    )
    ...
    extract_images_text: bool = Field(
        default=False, description="Extract text from images within PDF"
    )
...
class PDFToTextService:
    ...
    def extract_text_from_file(
        self, file_path: str, options: Optional[PDFExtractOptions] = None
    ) -> PDFTextResult:
        """
        ...
        if file_ext not in self.supported_formats:
            raise ValueError(
                f"Unsupported file format: {file_ext}. Supported formats: {self.supported_formats}"
            )
    ...
    def extract_text_from_bytes(
        self, pdf_bytes: bytes, options: Optional[PDFExtractOptions] = None
    ) -> PDFTextResult:
        """
    ...
    def _extract_text_from_document(
        self, doc: pymupdf.Document, options: PDFExtractOptions
    ) -> PDFTextResult:
        """
        ...
        should_use_vision = options.force_vision_processing or (
            options.use_vision_fallback and len(combined_text.strip()) < 50
    ...
    def _create_semantic_chunks_with_service(
        self, text: str, options: PDFExtractOptions
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_text_from_image_with_client(
        self, img_base64: str, options: PDFExtractOptions, vision_client
    ) -> Optional[str]:
...
def extract_with_vision_fallback(file_path: str) -> PDFTextResult:
    ...
    """
    service = PDFToTextService()
    options = PDFExtractOptions(use_vision_fallback=True)
...
def extract_tables_from_pdf(file_path: str) -> PDFTextResult:
    ...
    """
    service = PDFToTextService()
    options = PDFExtractOptions(force_vision_processing=True, extract_tables=True)
    return service.extract_text_from_file(file_path, options)
...
def extract_text_from_scanned_pdf(file_path: str) -> str:
    ...
    """
    service = PDFToTextService()
    options = PDFExtractOptions(force_vision_processing=True, extract_images_text=True)
    result = service.extract_text_from_file(file_path, options)
...
def extract_pdf_for_rag(
    ...
    if not CHUNKING_AVAILABLE:
        print(
            "Warning: Chunking service not available. Returning full text as single chunk."
        )
