...
class VisualExtractOptions(BaseModel):
    ...
    page_range: Optional[tuple] = Field(default=None, description="Page range as (start, end) tuple for PDFs")
    ...
    extract_images_text: bool = Field(default=False, description="Extract text from images within content")
    include_bounding_boxes: bool = Field(default=False, description="Include bounding box information for text elements")
    
    # Image processing options
    image_dpi: int = Field(default=150, description="DPI for PDF to image conversion")
    image_format: str = Field(default="png", description="Image format for PDF conversion")
...
class VisualToTextService:
    ...
    def extract_text_from_file(self, file_path: str, options: Optional[VisualExtractOptions] = None) -> VisualTextResult:
        """
        ...
        if file_ext in self.supported_pdf_formats:
            ...
            raise ValueError(f"Unsupported file format: {file_ext}. Supported formats: {self.supported_pdf_formats + self.supported_image_formats}")
    
    def extract_text_from_pdf_bytes(self, pdf_bytes: bytes, options: Optional[VisualExtractOptions] = None) -> VisualTextResult:
        """
    ...
    def extract_text_from_base64_image(self, base64_image: str, options: Optional[VisualExtractOptions] = None) -> VisualTextResult:
        """
        ...
        try:
            ...
            text = self._extract_text_from_image_with_client(base64_image, options, vision_client)
    ...
    def _extract_from_pdf(self, file_path: str, options: VisualExtractOptions) -> VisualTextResult:
    ...
    def _extract_from_image(self, file_path: str, options: VisualExtractOptions) -> VisualTextResult:
        ...
        try:
            ...
            # Extract text using vision model
            text = self._extract_text_from_image_with_client(img_base64, options, vision_client)
    ...
    def _extract_text_from_pdf_document(self, doc: pymupdf.Document, options: VisualExtractOptions) -> VisualTextResult:
        """
        ...
        for page_num in range(start_page, end_page):
            ...
            if VISION_AVAILABLE and (len(page_text.strip()) < 50 or options.include_bounding_boxes):
        ...
        if VISION_AVAILABLE and (len(combined_text.strip()) < 50 or options.include_bounding_boxes):
            try:
    ...
    def _process_pdf_with_vision(self, doc: pymupdf.Document, options: VisualExtractOptions) -> Optional[str]:
    ...
    def _extract_text_from_image_with_client(self, img_base64: str, options: VisualExtractOptions, 
    ...
    def _create_semantic_chunks_with_service(self, text: str, options: VisualExtractOptions) -> List[Dict[str, Any]]:
...
def extract_text_from_base64_image(base64_image: str, include_bounding_boxes: bool = False) -> str:
    """
...
def extract_with_bounding_boxes(file_path_or_base64: str, is_base64: bool = False) -> VisualTextResult:
    """
...
def extract_tables_from_visual(file_path_or_base64: str, is_base64: bool = False) -> VisualTextResult:
    """
