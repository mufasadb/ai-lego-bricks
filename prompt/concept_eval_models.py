...
class ConceptCheck(BaseModel):
    ...
    concept: str = Field(
        ..., description="The concept, question, or criteria to evaluate"
    )
    expected_value: Optional[str] = Field(
        None, description="For binary decisions, the expected answer (yes/no)"
    )
...
class EvalTestCase(BaseModel):
    ...
    context: Dict[str, Any] = Field(
        ..., description="Variables to fill the prompt template"
    )
    concept_checks: List[ConceptCheck] = Field(
        ..., description="Checks to perform on the output"
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Additional metadata"
    )
    expected_output: Optional[str] = Field(
        None, description="Optional expected output for reference"
    )
...
class PromptEvaluation(BaseModel):
    ...
    judge_max_tokens: Optional[int] = Field(
        None, description="Max tokens for judge responses"
    )
...
class TestCaseResult(BaseModel):
    ...
    check_results: List[CheckResult] = Field(
        ..., description="Results of all concept checks"
    )
...
class EvalExecutionResult(BaseModel):
    ...
    concept_breakdown: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Pass rate and details for each concept type"
    )
...
class ConceptEvalDefinition(BaseModel):
    ...
    test_cases: List[Dict[str, Any]] = Field(
        ..., description="Simplified test case definitions"
    )
    concept_checks: List[Dict[str, Any]] = Field(
        ..., description="Reusable concept check definitions"
    )
        ...
        for test_case_def in self.test_cases:
            ...
            for check_ref in test_case_def.get("concept_checks", []):
                if isinstance(check_ref, str):
                    ...
                    check_def = next(
                        (c for c in self.concept_checks if c.get("id") == check_ref),
                        None,
                    )
                    if check_def:
                        checks.append(
                            ConceptCheck(
                                **{k: v for k, v in check_def.items() if k != "id"}
                            )
                        )
                else:
