...
class ConceptEvalStorageBackend(ABC):
    ...
    @abstractmethod
    def get_evaluation_definition(self, eval_id: str) -> Optional[ConceptEvalDefinition]:
        """Retrieve an evaluation definition by ID"""
        pass
    
    @abstractmethod
    def list_evaluation_definitions(self, tags: Optional[List[str]] = None) -> List[ConceptEvalDefinition]:
    ...
    @abstractmethod
    def get_execution_results(self, eval_id: str, limit: int = 50) -> List[EvalExecutionResult]:
        """Get execution results for an evaluation"""
        pass
    
    @abstractmethod
    def get_latest_execution_result(self, eval_id: str) -> Optional[EvalExecutionResult]:
...
class SupabaseConceptEvalStorage(ConceptEvalStorageBackend):
    ...
    def get_evaluation_definition(self, eval_id: str) -> Optional[ConceptEvalDefinition]:
        ...
        try:
            result = self.client.table(self.definitions_table).select("*").eq("eval_id", eval_id).execute()
    ...
    def delete_evaluation_definition(self, eval_id: str) -> bool:
        ...
        try:
            self.client.table(self.definitions_table).delete().eq("eval_id", eval_id).execute()
            return True
        except Exception as e:
    ...
    def save_execution_result(self, result: EvalExecutionResult) -> bool:
        ...
        try:
            ...
            data['test_case_results'] = json.dumps(data['test_case_results'], default=str)
    ...
    def get_execution_results(self, eval_id: str, limit: int = 50) -> List[EvalExecutionResult]:
        ...
        try:
            result = (self.client.table(self.results_table)
                     .select("*")
...
def create_concept_eval_storage(backend_type: str = "auto", **kwargs) -> ConceptEvalStorageBackend:
    """
    ...
    if backend_type == "file":
        return FileConceptEvalStorage(kwargs.get('storage_path', './concept_evaluations'))
