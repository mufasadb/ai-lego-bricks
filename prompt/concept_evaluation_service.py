...
class ConceptEvaluationService:
    ...
    def run_evaluation(
        ...
        for i, test_case in enumerate(evaluation.test_cases):
            ...
            result = self._run_test_case(
                evaluation.prompt_template, test_case, llm_client, judge_service
            )
        ...
        recommendations = self._generate_recommendations(
            test_case_results, summary_metrics
        )
    ...
    def run_evaluation_by_id(
        self, eval_id: str, **kwargs
    ...
    def _run_test_case(
        ...
        llm_client,
        judge_service: ConceptJudgeService,
    ) -> TestCaseResult:
        ...
        try:
            ...
            check_results = judge_service.evaluate_multiple_checks(
                llm_output, test_case.concept_checks
            )

            # Calculate overall results
            overall_passed = all(result.passed for result in check_results)
            overall_score = self._calculate_test_case_score(
                check_results, test_case.concept_checks
            )
    ...
    def _calculate_test_case_score(
        self, check_results: List[CheckResult], concept_checks: List[ConceptCheck]
    ) -> float:
    ...
    def _calculate_concept_breakdown(
        self, test_case_results: List[TestCaseResult]
        ...
        for check_type, stats in concept_stats.items():
            ...
            for concept, concept_stats in stats["concepts"].items():
                ...
                concept_rate = (
                    concept_passed / concept_total if concept_total > 0 else 0.0
                )
    ...
    def _generate_recommendations(
        self, test_case_results: List[TestCaseResult], summary_metrics: Dict[str, Any]
    ) -> List[str]:
        ...
        if overall_score < 0.7:
            recommendations.append(
                f"Overall score is {overall_score:.1%}. Consider revising the prompt template for better performance."
            )

        if failed_cases > total_cases * 0.3:
            recommendations.append(
                f"{failed_cases}/{total_cases} test cases failed. Review failed cases for common patterns."
            )
        ...
        if failure_patterns:
            ...
            recommendations.append(
                f"Most common failure: '{most_common_failure[0]}' failed {most_common_failure[1]} times."
            )
        ...
        if avg_time > 5000:  # 5 seconds
            recommendations.append(
                f"Average execution time is {avg_time:.0f}ms. Consider optimizing prompt length."
            )

        if not recommendations:
            recommendations.append(
                "✨ Evaluation performing well! All metrics are within acceptable ranges."
            )
