...
class ConceptJudgeService:
    ...
    def __init__(self, judge_model: str = "gemini", temperature: float = 0.1, max_tokens: int = 500):
        """
    ...
    def evaluate_concept_check(self, output: str, concept_check: ConceptCheck) -> CheckResult:
        """
        ...
        try:
            ...
            # Parse the response
            passed, reasoning, confidence = self._parse_judge_response(response, concept_check.type)
    ...
    def evaluate_multiple_checks(self, output: str, concept_checks: List[ConceptCheck]) -> List[CheckResult]:
        """
...
CONFIDENCE: [A number from 0.0 to 1.0 indicating how confident you are in your assessment]"""
    ...
    def _get_must_not_contain_prompt(self, output: str, concept_check: ConceptCheck) -> str:
...
CONFIDENCE: [A number from 0.0 to 1.0 indicating how confident you are in your assessment]"""
    ...
    def _get_binary_decision_prompt(self, output: str, concept_check: ConceptCheck) -> str:
...
CONFIDENCE: [A number from 0.0 to 1.0 indicating how confident you are in your assessment]"""
    ...
    def _parse_judge_response(self, response: str, check_type: ConceptCheckType) -> tuple[bool, str, float]:
        """
        ...
        try:
            ...
            passed = 'YES' in answer_upper and 'NO' not in answer_upper.replace('YES', '')
            ...
            reasoning = f"Parsed from response: {response[:200]}..." if len(response) > 200 else response
            confidence = 0.5
...
class BatchConceptJudge:
    ...
    def evaluate_batch(self, outputs: List[str], concept_checks: List[ConceptCheck]) -> List[List[CheckResult]]:
        """
        ...
        for output in outputs:
            output_results = self.judge_service.evaluate_multiple_checks(output, concept_checks)
            batch_results.append(output_results)
        
        return batch_results
    
    def evaluate_single_concept_batch(self, outputs: List[str], concept_check: ConceptCheck) -> List[CheckResult]:
        """
