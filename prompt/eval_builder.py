...
from .concept_eval_models import ConceptEvalDefinition, COMMON_CONCEPT_CHECKS
...
class EvaluationBuilder:
    ...
    def add_common_check(
        self, check_name: str, check_id: Optional[str] = None
    ) -> "EvaluationBuilder":
        """
        ...
        if check_name not in COMMON_CONCEPT_CHECKS:
            raise ValueError(
                f"Unknown common check: {check_name}. Available: {list(COMMON_CONCEPT_CHECKS.keys())}"
            )
    ...
    def add_test_case(
        ...
        **metadata,
        ...
        """
        test_case = {"context": context, "concept_checks": concept_check_refs}
    ...
    def add_test_case_with_inline_checks(
        self,
        context: Dict[str, Any],
        ...
        **metadata,
        ...
        Args:
            ...
            **metadata: Additional metadata for the test case
        """
        test_case = {"context": context, "concept_checks": inline_checks}
...
class QuickEvaluationBuilder:
    ...
    @staticmethod
    def create_accuracy_evaluation(
        name: str,
        prompt_template: str,
    ...
    @staticmethod
    def create_binary_decision_evaluation(
        name: str,
        prompt_template: str,
...
class EvaluationImporter:
    ...
    def from_csv(
        ...
        with open(csv_path, "r", encoding="utf-8") as f:
            ...
            check_refs = [
                check.get("id", f"check_{i}") for i, check in enumerate(concept_checks)
            ]

            for i, row in enumerate(reader):
                context = {col: row[col] for col in context_columns if col in row}
                expected_output = (
                    row.get(expected_output_column) if expected_output_column else None
                )
...
class EvaluationTemplates:
    ...
    @staticmethod
    def get_classification_eval(
        categories: List[str], test_cases: List[Dict[str, Any]]
    ) -> ConceptEvalDefinition:
        """Template for evaluating classification prompts"""
        builder = EvaluationBuilder(
            "Classification Evaluation", "Evaluates classification accuracy"
        )
