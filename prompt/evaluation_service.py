...
class EvaluationService:
    ...
    def calculate_prompt_metrics(
        self, prompt_id: str, version: Optional[str] = None, days_back: int = 7
        ...
        period_executions = [
            e for e in executions if start_time <= e.created_at <= end_time
        ]
        ...
        success_rate = (
            len(successful_executions) / total_executions if total_executions > 0 else 0
        )
        error_rate = (
            len(failed_executions) / total_executions if total_executions > 0 else 0
        )
    ...
    def _calculate_performance_score(
        self, success_rate: float, avg_response_time: float, avg_tokens: float
    ) -> float:
        """
    ...
    def compare_prompt_versions(
        ...
        metrics_a = self.calculate_prompt_metrics(
            prompt_a_id, prompt_a_version, days_back
        )
        metrics_b = self.calculate_prompt_metrics(
            prompt_b_id, prompt_b_version, days_back
        )
        ...
        response_time_diff = (
            metrics_a.average_response_time_ms - metrics_b.average_response_time_ms
        )  # Lower is better
    ...
    def identify_performance_regressions(
        self, threshold: float = 10.0, days_back: int = 7
    ) -> List[Dict[str, Any]]:
        """
    ...
    def generate_evaluation_report(
        ...
        if metrics.success_rate < 0.95:
            recommendations.append(
                f"Success rate is {metrics.success_rate:.1%}. Consider reviewing error patterns."
            )

        if metrics.average_response_time_ms > 3000:
            recommendations.append(
                f"Average response time is {metrics.average_response_time_ms:.0f}ms. Consider optimizing prompt length."
            )

        if metrics.error_rate > 0.1:
            recommendations.append(
                f"Error rate is {metrics.error_rate:.1%}. Review common errors: {', '.join(metrics.common_errors[:3])}"
            )
        ...
        if avg_tokens > 3000:
            recommendations.append(
                f"High token usage ({avg_tokens:.0f} avg). Consider making prompt more concise."
            )
