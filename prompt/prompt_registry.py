...
class PromptCache:
    ...
    def invalidate(self, prompt_id: str, version: Optional[str] = None) -> None:
        ...
        if version:
            ...
            with self._lock:
                keys_to_remove = [k for k in self._cache.keys() if k.startswith(f"{prompt_id}::")]
...
class PromptRegistry:
    ...
    def validate_prompt_reference(self, prompt_id: str, version: Optional[str] = None) -> bool:
    ...
    def get_prompt_by_name(self, name: str, version: Optional[str] = None) -> Optional[Prompt]:
