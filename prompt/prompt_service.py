...
class PromptService:
    ...
    def __init__(
        self,
        storage_backend: Optional[PromptStorageBackend] = None,
        cache_ttl: int = 3600,
    ):
        """
    ...
    def create_prompt(
        ...
        for content_item in content:
            ...
            if (
                isinstance(content_item["content"], dict)
                and "template" in content_item["content"]
            ):
    ...
    def create_prompt_version(
        ...
        if not self.registry.register_prompt(new_prompt):
            raise RuntimeError(
                f"Failed to register new version {new_version} for prompt {prompt_id}"
            )
    ...
    def activate_prompt(self, prompt_id: str, version: Optional[str] = None) -> bool:
        ...
        if current_active:
            self.registry.update_prompt_status(
                prompt_id, PromptStatus.DEPRECATED, current_active.version.version
            )

        # Activate the specified version
        return self.registry.update_prompt_status(
            prompt_id, PromptStatus.ACTIVE, version
        )

    def deactivate_prompt(self, prompt_id: str, version: Optional[str] = None) -> bool:
        """Deactivate a prompt version"""
        return self.registry.update_prompt_status(
            prompt_id, PromptStatus.DEPRECATED, version
        )
    ...
    def validate_template(
        self, template: str, required_variables: List[str], test_context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            prompt_template = PromptTemplate(
                template=template, required_variables=required_variables
            )

            # Try to render with test context
            rendered = prompt_template.render(test_context)

            return {"valid": True, "rendered": rendered, "errors": []}
        except Exception as e:
            return {"valid": False, "rendered": None, "errors": [str(e)]}
    ...
    def export_training_data(
        ...
        if format == "jsonl":
            ...
            # Header
            writer.writerow(
                ["prompt_id", "version", "input", "output", "execution_time_ms"]
            )
            ...
            for execution in executions:
                if execution.success and execution.llm_response:
                    ...
                    input_text = " ".join(
                        [msg["content"] for msg in execution.rendered_messages]
                    )
                    writer.writerow(
                        [
                            execution.prompt_id,
