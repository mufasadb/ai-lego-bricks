...
class PromptStorageBackend(ABC):
    ...
    @abstractmethod
    def get_executions(
        self, prompt_id: str, version: Optional[str] = None, limit: int = 100
...
class SupabasePromptStorage(PromptStorageBackend):
    ...
    def _ensure_tables(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    "Warning: prompts table doesn't exist. Please create it manually or use Supabase migrations."
                )
                print("Required schema:")
                print(
                    "- prompts table: id, name, content (jsonb), metadata (jsonb), version (jsonb), status, created_at, updated_at, parent_id"
                )
                print(
                    "- prompt_executions table: id, prompt_id, prompt_version, execution_context (jsonb), rendered_messages (jsonb), llm_provider, llm_model, llm_response, execution_time_ms, token_usage (jsonb), success, error_message, metadata (jsonb), created_at"
                )
        ...
        try:
            ...
            print(
                "Warning: prompt_executions table doesn't exist. Please create it manually or use Supabase migrations."
            )
    ...
    def save_execution(self, execution: PromptExecution) -> bool:
        ...
        try:
            ...
            execution_data["execution_context"] = json.dumps(
                execution_data["execution_context"]
            )
            execution_data["rendered_messages"] = json.dumps(
                execution_data["rendered_messages"]
            )
            execution_data["token_usage"] = (
                json.dumps(execution_data["token_usage"])
                if execution_data["token_usage"]
                else None
            )
            execution_data["metadata"] = json.dumps(execution_data["metadata"])

            result = (
                self.client.table("prompt_executions").insert(execution_data).execute()
            )
    ...
    def get_executions(
        ...
        try:
            query = (
                self.client.table("prompt_executions")
                .select("*")
            ...
            for data in result.data:
                ...
                data["token_usage"] = (
                    json.loads(data["token_usage"]) if data["token_usage"] else None
                )
                data["metadata"] = json.loads(data["metadata"])
