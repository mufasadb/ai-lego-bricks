...
class HttpRequestConfig:
    ...
    def __post_init__(self):
        ...
        if sum(x is not None for x in data_fields) > 1:
            raise ValueError("Only one of json_data, form_data, or data can be specified")
...
class HttpRequestService:
    ...
    async def request(self, config: HttpRequestConfig) -> HttpResponse:
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                logger.info(f"HTTP {config.method.value} {config.url} -> {response.status_code} ({elapsed_time:.2f}s)")
                ...
                if attempt < self.max_retries:
                    ...
                    logger.warning(f"HTTP request failed (attempt {attempt + 1}/{self.max_retries + 1}): {e}. Retrying in {delay}s...")
                    await asyncio.sleep(delay)
                else:
                    logger.error(f"HTTP request failed after {self.max_retries + 1} attempts: {e}")
                    raise
    ...
    async def delete(self, url: str, headers: Optional[Dict[str, str]] = None, **kwargs) -> HttpResponse:
    ...
    async def head(self, url: str, headers: Optional[Dict[str, str]] = None, **kwargs) -> HttpResponse:
    ...
    async def options(self, url: str, headers: Optional[Dict[str, str]] = None, **kwargs) -> HttpResponse:
...
def create_http_request_service(credential_manager: Optional[CredentialManager] = None,
                               **kwargs) -> HttpRequestService:
    """
