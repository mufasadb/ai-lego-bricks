...
class FasterWhisperClient(STTClient):
    ...
    def __init__(self, config: STTConfig, credential_manager: Optional['CredentialManager'] = None):
        ...
        self.server_url = config.extra_params.get("server_url") or "http://localhost:10300"
        self.timeout = config.extra_params.get("timeout", 120)  # Longer timeout for transcription
...
class GoogleSTTClient(STTClient):
    ...
    def speech_to_text(self, audio_file_path: str, **kwargs) -> STTResponse:
        ...
        enable_word_timestamps = kwargs.get("enable_word_timestamps", self.config.enable_word_timestamps)
        enable_speaker_diarization = kwargs.get("enable_speaker_diarization", self.config.enable_speaker_diarization)
    ...
    def get_supported_languages(self) -> List[str]:
        ...
        if self._languages_cache is None:
            ...
            self._languages_cache = [
                ...
                'ar-SA', 'hi-IN', 'tr-TR', 'pl-PL', 'nl-NL', 'sv-SE', 'da-DK',
                'no-NO', 'fi-FI', 'th-TH', 'vi-VN', 'id-ID', 'ms-MY', 'he-IL'
            ]
