...
class STTServiceFactory:
    ...
    def create_stt_service(
        provider: Union[str, STTProvider] = "auto",
        ...
        **kwargs,
        ...
        if provider == STTProvider.FASTER_WHISPER:
            client = STTServiceFactory._create_faster_whisper_client(
    ...
    def _create_config(provider: STTProvider, **kwargs) -> STTConfig:
        ...
        if provider == STTProvider.FASTER_WHISPER:
            config_dict["extra_params"].update(
                {
                    "server_url": kwargs.get(
                        "server_url",
                        os.getenv("FASTER_WHISPER_URL", "http://localhost:10300"),
                    ),
            ...
            if not config_dict["language"]:
                config_dict["language"] = os.getenv(
            ...
            config_dict["extra_params"].update(
                {
                    ...
                    "enable_profanity_filter": kwargs.get(
            ...
            if not config_dict["language"]:
                config_dict["language"] = os.getenv(
                    "GOOGLE_STT_DEFAULT_LANGUAGE", "en-US"
                )
    ...
    @staticmethod
    def _create_faster_whisper_client(
        config: STTConfig, credential_manager: Optional["CredentialManager"] = None
    ) -> FasterWhisperClient:
    ...
    def get_available_providers() -> Dict[str, bool]:
        ...
        providers = {"faster_whisper": False, "google": False}

        # Check Faster Whisper
        faster_whisper_url = os.getenv("FASTER_WHISPER_URL", "http://localhost:10300")
        providers["faster_whisper"] = (
            STTServiceFactory._check_faster_whisper_availability(faster_whisper_url)
        )
    ...
    def get_provider_info() -> Dict[str, Any]:
        ...
        return {
            "faster_whisper": {
                ...
                "available": STTServiceFactory.get_available_providers()[
                    "faster_whisper"
                ],
