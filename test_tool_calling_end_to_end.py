...
class WeatherExecutor(ToolExecutor):
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        try:
            ...
            weather = weather_data.get(location, {'temp': '20°C', 'condition': 'Pleasant', 'humidity': '70%'})
...
async def test_tool_preparation():
    ...
    try:
        ...
        for provider in providers:
            tool_config = await tool_service.prepare_tools_for_provider(provider, tool_names)
            print(f"  {provider}: {len(tool_config.get('tools', tool_config.get('functionDeclarations', [])))} tools prepared")
        ...
        if result.step_outputs:
            for key, value in result.step_outputs.items():
                if isinstance(value, dict):
                    ...
                    if response:
                        print(f"  Final Response: {response[:200]}{'...' if len(response) > 200 else ''}")
        
        # Determine success
        step_result = list(result.step_outputs.values())[0] if result.step_outputs else {}
