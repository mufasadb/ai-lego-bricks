...
class MCPConfigManager:
                ...
                if path.exists():
                    ...
                    for name, config in mcp_servers.items():
                        try:
                            ...
                            logger.error(f"Failed to parse Claude Desktop server '{name}': {e}")
                    
                    break  # Use first found config
                    
            except Exception as e:
                logger.debug(f"Could not read Claude Desktop config from {config_path}: {e}")
    ...
    def _parse_claude_desktop_server(self, name: str, config: Dict[str, Any]) -> MCPServerConfig:
...
async def initialize_mcp_servers_from_config(
    ...
    logger.info(f"MCP server initialization complete: {len(results['servers'])} started, {len(results['errors'])} failed")
    return results
