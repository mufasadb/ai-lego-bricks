...
class MCPServerProcess:
    ...
    def __init__(self, config: MCPServerConfig, credential_manager: Optional[CredentialManager] = None):
        self.config = config
    ...
    def _validate_credentials(self):
        ...
        if not self.credential_manager:
            if CredentialManager:
                ...
                raise ValueError(f"Server '{self.config.name}' requires credentials but no credential manager available")
        ...
        if self.config.env_credentials and self.credential_manager:
            for env_var, cred_key in self.config.env_credentials.items():
                ...
                if cred_value:
                    ...
                    logger.warning(f"Credential '{cred_key}' not found for env var '{env_var}'")
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            logger.info(f"Starting MCP server '{self.config.name}' with command: {command}")
    ...
    async def send_request(self, method: str, params: Optional[Dict[str, Any]] = None, 
        ...
        try:
            ...
            logger.error(f"Failed to send request {method} to '{self.config.name}': {e}")
            raise
        finally:
        ...
        try:
            while self.process and self.process.returncode is None:
                ...
                try:
                    ...
                    logger.error(f"Invalid JSON from MCP server '{self.config.name}': {e}")
                except Exception as e:
    ...
    async def _handle_response(self, response_data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if response.id and response.id in self._pending_requests:
                ...
                if not future.done():
                    if response.error:
                        error = Exception(f"MCP Error {response.error['code']}: {response.error['message']}")
                        future.set_exception(error)
                    else:
                        future.set_result(response.result)
            else:
                # Handle notification or unexpected response
                logger.debug(f"Received notification from '{self.config.name}': {response_data}")
...
class MCPServerManager:
    ...
    def __init__(self, credential_manager: Optional[CredentialManager] = None):
        ...
        self.credential_manager = credential_manager or (CredentialManager() if CredentialManager else None)
