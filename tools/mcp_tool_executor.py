...
class MCPToolExecutor(SecureToolExecutor):
    ...
    def __init__(self, server_name: str, tool_name: str, credential_manager=None):
        """
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            result = await manager.call_tool_on_server(
                self.server_name, self.tool_name, tool_call.parameters
            )
            ...
            logger.error(
                f"Error executing MCP tool '{self.tool_name}' on server '{self.server_name}': {e}"
            )
            return ToolResult(
                tool_call_id=tool_call.id,
                name=tool_call.name,
...
class MCPToolDiscovery:
    ...
    async def discover_tools_from_server(server_name: str) -> List[Tool]:
        ...
        try:
            ...
            if not server or not server.is_initialized:
                logger.warning(
                    f"MCP server '{server_name}' is not available for tool discovery"
                )
                return tools
            ...
            for mcp_tool_info in mcp_tools:
                try:
                    tool = MCPToolDiscovery._convert_mcp_tool_to_tool(
                        server_name, mcp_tool_info
                    )
                    tools.append(tool)
                except Exception as e:
                    logger.error(
                        f"Failed to convert MCP tool '{mcp_tool_info.get('name', 'unknown')}': {e}"
                    )
            ...
            logger.error(
                f"Failed to discover tools from MCP server '{server_name}': {e}"
            )

        return tools

    @staticmethod
    def _convert_mcp_tool_to_tool(
        server_name: str, mcp_tool_info: Dict[str, Any]
    ) -> Tool:
    ...
    def _convert_schema_to_tool_parameter(
        ...
        if param_type == ParameterType.ARRAY and "items" in schema:
            param.items = MCPToolDiscovery._convert_schema_to_tool_parameter(
                schema["items"]
            )
        ...
        if param_type == ParameterType.OBJECT and "properties" in schema:
            ...
            for prop_name, prop_schema in schema["properties"].items():
                properties[prop_name] = (
                    MCPToolDiscovery._convert_schema_to_tool_parameter(prop_schema)
                )
...
async def register_mcp_tools_globally(
    ...
    results = {"registered": [], "failed": [], "total": 0}
    ...
    for server_name in server_names:
        try:
            ...
            logger.error(
                f"Failed to register tools from MCP server '{server_name}': {e}"
            )
            results["failed"].append({"server": server_name, "error": str(e)})

    logger.info(
        f"MCP tool registration complete: {len(results['registered'])} registered, {len(results['failed'])} failed"
    )
    return results
