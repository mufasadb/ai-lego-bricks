...
class SlackWebhookExecutor(WebhookToolExecutor):
    ...
    def __init__(self, credential_manager: Optional[CredentialManager] = None):
        super().__init__(
            webhook_url_key="SLACK_WEBHOOK_URL", credential_manager=credential_manager
        )
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            data = {"text": message, "channel": channel, "username": username}
            ...
            parameters=ToolParameter(
                ...
                properties={
                    "message": ToolParameter(
                        type=ParameterType.STRING, description="The message to send"
                    ),
                ...
                if query_type == "select":
                    async with session.get(url, headers=headers) as response:
                        if response.status >= 400:
                            ...
                            raise Exception(
                                f"Supabase query failed: {response.status} - {error_text}"
                            )
    ...
    def get_schema(cls) -> ToolSchema:
        return ToolSchema(
            ...
            parameters=ToolParameter(
                ...
                properties={
                    "table": ToolParameter(
                        type=ParameterType.STRING, description="Table name to query"
                    ),
    ...
    async def execute(self, tool_call: ToolCall) -> ToolResult:
        ...
        try:
            ...
            if operation == "get_repo":
                ...
                if not repo:
                    raise ValueError(
                        "repo parameter required for list_issues operation"
                    )
...
def create_secure_example_tools(
    credential_manager: Optional[CredentialManager] = None,
) -> Dict[str, Tool]:
