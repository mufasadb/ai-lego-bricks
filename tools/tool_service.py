...
class ToolService:
    ...
    async def prepare_tool_choice_for_provider(
        self, provider: str, tool_choice: Optional[ToolChoice] = None
    ) -> Any:
    ...
    async def format_tool_results_for_provider(
        self, provider: str, results: List[ToolResult]
    ) -> List[Dict[str, Any]]:
    ...
    async def get_tool_calling_config(
        ...
        # Add tools
        tools_config = await self.prepare_tools_for_provider(
            provider, tool_names, category
        )
        config.update(tools_config)

        # Add tool choice
        if tools_config:  # Only add tool choice if tools are available
            choice_config = await self.prepare_tool_choice_for_provider(
    ...
    async def process_tool_calling_workflow(
        ...
        formatted_results = await self.format_tool_results_for_provider(
            provider, results
        )
    ...
    async def register_secure_tool(
        self,
        tool: Tool,
        category: Optional[str] = None,
    ...
    async def register_tools_with_credentials(
        ...
        """
        results = {"registered": [], "failed": [], "total": len(tool_configs)}
    ...
    async def validate_tool_credentials(
        ...
        for tool_name in tools_to_check:
            ...
            if tool_result["available"]:
                ...
                results["missing_credentials"].extend(
                    tool_result["missing_credentials"]
                )
