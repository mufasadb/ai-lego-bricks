...
class StreamingTTSService:
    ...
    def stream_text_to_audio(self, text_generator: Generator[str, None, str], 
                           output_dir: str = "output/streaming_audio",
                           voice: Optional[str] = None) -> Generator[Dict[str, Any], None, List[str]]:
        """
            ...
            for chunk in text_generator:
                ...
                if sentences:
                    ...
                    if (len(self.sentence_buffer) >= self.sentence_buffer_size or 
                        time.time() - self.last_buffer_time > self.max_buffer_time):
    ...
    def _audio_worker(self, output_dir: str, voice: Optional[str]):
        ...
        while not self.stop_audio_thread:
            try:
                ...
                if text:
                    audio_count += 1
                    output_path = os.path.join(output_dir, f"chunk_{audio_count:03d}.wav")
...
class StreamingLLMToTTSPipeline:
    ...
    def __init__(self, llm_service, tts_service: TTSService, 
    ...
    def stream_chat_to_audio(self, message: str, voice: Optional[str] = None,
                           progress_callback: Optional[Callable[[Dict[str, Any]], None]] = None) -> Generator[Dict[str, Any], None, Dict[str, Any]]:
        """
        ...
        for progress in self.streaming_tts.stream_text_to_audio(
            text_stream, 
            output_dir=self.streaming_config["output_dir"],
            voice=voice
        ):
            # Track complete text
...
def create_streaming_pipeline(llm_provider: str = "ollama", 
...
def quick_stream_chat_to_audio(message: str, 
                             llm_provider: str = "ollama",
                             tts_provider: str = "auto",
                             voice: Optional[str] = None,
                             output_dir: str = "output/quick_streaming") -> Dict[str, Any]:
    """
    ...
    for progress in pipeline.stream_chat_to_audio(message, voice):
        print(f"Status: {progress['status']}, Audio files: {progress.get('audio_files_generated', 0)}")
        result = progress
