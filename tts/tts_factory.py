...
class TTSServiceFactory:
    ...
    def create_tts_service(
        provider: Union[str, TTSProvider] = "auto",
        ...
        **kwargs,
        ...
        if provider == TTSProvider.COQUI_XTTS:
            client = TTSServiceFactory._create_coqui_xtts_client(
            ...
            config_dict["extra_params"].update(
                {
                    ...
                    "language": kwargs.get(
                        "language", os.getenv("COQUI_XTTS_DEFAULT_LANGUAGE", "en")
                    ),
                }
            )
            if not config_dict["voice"]:
                config_dict["voice"] = os.getenv(
    ...
    @staticmethod
    def _create_coqui_xtts_client(
        config: TTSConfig, credential_manager: Optional["CredentialManager"] = None
    ) -> CoquiXTTSClient:
    ...
    def get_available_providers() -> Dict[str, bool]:
        ...
        providers = {"coqui_xtts": False, "openai": False, "google": False}

        # Check Coqui-XTTS
        coqui_url = os.getenv("COQUI_XTTS_URL")
        providers["coqui_xtts"] = bool(
            coqui_url
        ) and TTSServiceFactory._check_coqui_xtts_availability(coqui_url)
