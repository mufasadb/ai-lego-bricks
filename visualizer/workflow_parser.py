...
class WorkflowParser:
    ...
    def __init__(self):
        self.step_types = {
            "input", "document_processing", "memory_store", "memory_retrieve",
            "llm_chat", "llm_structured", "llm_vision", "chunk_text",
            "condition", "loop", "output", "file_output", "human_approval",
            "concept_evaluation", "prompt_management", "streaming_chat",
            "tts_generation", "conversation_management"
        }
    ...
    def _add_sequential_connections(self, steps: List[StepInfo], connections: List[Dict[str, str]]):
        ...
        for i in range(len(steps) - 1):
            ...
            if (next_step.id not in connected_steps and 
                not next_step.inputs and 
                current_step.type not in ["condition", "loop"]):
    ...
    def _find_entry_points(self, steps: List[StepInfo], connections: List[Dict[str, str]]) -> List[str]:
    ...
    def _find_exit_points(self, steps: List[StepInfo], connections: List[Dict[str, str]]) -> List[str]:
    ...
    def get_step_statistics(self, workflow: WorkflowStructure) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_loops": any(step.type == "loop" or step.routes for step in workflow.steps),
